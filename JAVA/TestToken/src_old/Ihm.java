        		...
        		btnSubmit.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						if(!Arrays.equals(pass1,pass2)){
							...
							try {
								//generatedPassword = salt+"$"+h.hashFunction(pass+salt);
				...
				if(logUser.length()==0){
					...
					if(f.verifFingerFile(logUser)==0){
						try {
							...
							//System.out.println(finger);
							getProjet().procedureJAVA("Q:SELECT:Connexion:"+logUser+";"+hashPass+";"+finger);
					...
					*/
					...
					if (goodCred==1){
						...
						argsRecup=r.deparseArgs(getProjet().getRequestReturn());
    	      			...
    	      			try {
    	      				System.out.println("taille : " + en.encrypt("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", en.generateValidKey(pass)).length());
			...
			public void actionPerformed(ActionEvent e) {
			    ...
			    if(passDelete.length==0){
			    	...
			    	try {
						...
						getProjet().procedureJAVA("Q:SELECT:PreSupprUser:"+nameDelete+";"+hash);
					} catch (Exception e1) {
						e1.printStackTrace();
					}
			    }
